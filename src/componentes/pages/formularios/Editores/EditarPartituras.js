...
export const EditarPartituras = () => {
  ...
  useEffect(() => {
    const fetchFoto = async () => {
      const url = `https://backend-prueba-apel.onrender.com/api/partituras/${id}`;
  ...
  const guardar_foto = async (e) => {
    ...
    if (datos.status == "success") {
      ...
      const { subida2, cargando2 } = await Api("https://backend-prueba-apel.onrender.com/api/partituras/registrar-imagen/" + id, "POST", formData, true);
      const { subida, cargando } = await Api("https://backend-google-fnsu.onrender.com/api/partituras/editar-imagen/" + id, "POST", formData, true);
