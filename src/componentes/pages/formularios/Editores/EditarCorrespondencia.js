...
export const EditarCorrespondencia = () => {
  ...
  const guardar_foto = async (e) => {
    ...
    if (datos.status == "success") {
      ...
      const { subida3, cargando3 } = await Api("https://backend-google-fnsu.onrender.com/api/correspondencia/editar/" + id, "PUT", nueva_foto);
      const { subida, cargando } = await Api("https://backend-google-fnsu.onrender.com/api/correspondencia/registrar-imagen/" + id, "POST", formData, true);
