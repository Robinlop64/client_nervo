...
export const RegVideo = () => {
    ...
    const guardar_video = async (e) => {
        ...
        const { datos } = await Api("https://backend-prueba-apel.onrender.com/api/videos/registrar", "POST", formulario);
        ...
        if (datos.status === "success" && datos.publicacionGuardada?._id) {
            ...
            const subidaVideo = await Api(
                `https://backend-prueba-apel.onrender.com/api/videos/cargar-videos/${datos.publicacionGuardada._id}`,
                "POST",
                videoFormData,
                true
            );
